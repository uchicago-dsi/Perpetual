...
def visual_prepare(points, dis, route_dir, output_dir):
    ...
    for route_file in glob.glob(route_files_pattern):
        ...
        output_file_path = os.path.join(
            output_dir, os.path.basename(route_file).replace(".csv", "_vis.csv")
        )
...
def add_accumulate_value(df_route, dis):
    ...
    accumulate_col(df_route, "Daily_Pickup_Totes", number_of_rows, "Truck_Pickup_Load")

    accumulate_col(
        df_route, "Weekly_Dropoff_Totes", number_of_rows, "Truck_Dropoff_Load"
    )
