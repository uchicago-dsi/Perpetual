...
def parse_route_data(route_data):
    ...
    for key in route_dict.keys():
        if len(route_dict[key]) > 25:
            route_dict[key] = [
                route_dict[key][i : i + 25]
                for i in range(0, len(route_dict[key]), 25)
            ]
        else:
...
def parse_supplemental_data(datafile):
    ...
    lat_long_dict = dict(
        zip(df.index, df[["Latitude", "Longitude"]].values.tolist())
    )
    ...
    for truck_route in route_dict.keys():
        ...
        for route in route_dict[truck_route]:
            ...
            for i in route:
                ...
                coordinates = (
                    lat_long_dict[stop_index][1],
                    lat_long_dict[stop_index][0],
                )
            ...
            for i in route:
                ...
                if category == "outdoor":
                    ...
                    folium.Marker(
                        location=coordinates, popup=popup, icon=icon_dot
                    ).add_to(m)
                elif category == "depot":
                    folium.Marker(
                        coordinates, popup=popup, icon=folium.Icon(icon="star")
                    ).add_to(m)
                else:
                    icon_dot = folium.plugins.BeautifyIcon(
                        border_color="#000300",
            ...
            route = folium.PolyLine(
                locations=coords, color=route_color, weight=1
            )
            route.add_to(m)
...
def main():
    ...
    (
        ...
        pickup_capacity_dict,
        lat_long_dict,
    ) = parse_supplemental_data(data_file)
